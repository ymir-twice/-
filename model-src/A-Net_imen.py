...
def CLAHE(img):
    ...
    return trans(Image.fromarray(r_img))

def apply_clahe_to_batch(batch_tensor, device='cuda:0'):
    processed_images = []
    # 将每张图片单独处理
    ...
    # 将处理后的图片重新堆叠为一个 batch 张量
    return torch.stack(processed_images).to(torch.device(device))
